...
const RekomendasiController = {
  async getRekomendasi(request, reply) {
    const { budget, budget_pendaftaran, provinsi, kota, kurikulum, fasilitas, bobot } = request.body
    ...
    try {
      const data = await RekomendasiService.getRekomendasi({ budget, budget_pendaftaran, provinsi, kota, kurikulum, fasilitas, bobot })
